...
def plot_embedding(data, label):
    ...
    for i in range(data.shape[0]):
        plt.text(data[i, 0], data[i, 1], str(label[i]), color=plt.cm.Set1(label[i] / 15.),
...
parser.add_argument('--prc', default=0.02, type=float,
                    help='the weight of prc loss'
                    )

parser.add_argument('--src', default=0.01, type=float,
                    help='the weight of src loss'
                    )
...
# Directory for Log
LOG_DIR = args.LOG_DIR + '/logs_{}/{}_{}_embedding{}_alpha{}_mrg{}_{}_lr{}_batch{}{}_prc{}_src{}_seed{}'.format(args.dataset, args.model,
                                                                                             args.loss,
                                                                                             args.sz_embedding,
                                                                                             args.alpha,
                                                                                             ...
                                                                                             args.prc, args.src,
                                                                                             args.seed)
# Wandb Initialization /home/wangxinru/program/Proxy-Anchor-CVPR2020-master/logs
wandb.init(project="proxy")
...
if args.dataset != 'Inshop':
    ev_dataset = dataset.load(
        name=args.dataset,
        root=data_root,
        mode='eval',  # result = tsne.fit_transform(m.cpu().detach().numpy())
        # fig = plot_embedding(result,y.cpu().detach().numpy())
        # plt.show()
        transform=dataset.utils.make_transform(
            is_train=False,
            is_inception=(args.model == 'bn_inception')
...
if args.loss == 'Proxy_Anchor':
    criterion = losses.Proxy_Anchor(nb_classes=nb_classes, sz_embed=args.sz_embedding, mrg=args.mrg,
                                    alpha=args.alpha, prc=args.prc, src=args.src).cuda()
...
if args.loss == 'Proxy_Anchor':
    param_groups.append({'params': criterion.proxies, 'lr': float(args.lr) * 1000})
...
for epoch in range(0, args.nb_epochs):
    ...
    for batch_idx, (x, y) in pbar:
        ...
        # result = tsne.fit_transform(m.cpu().detach().numpy())
        # fig = plot_embedding(result,y.cpu().detach().numpy())
        # plt.show()
    ...
    if (epoch >= 0):
        ...
        if best_recall[0] < Recalls[0]:
            ...
            # torch.save({"proxy_dict": opt.param_groups[2]},
            #            "/home/wangxinru/program/Proxy-Anchor-CVPR2020-master/proxies")
            torch.save({"proxy_dict": opt.param_groups[2]},
                       "proxies")
